...
int xscugic_example(void)
{
	...
	Status = ScuGicExample(INTC_DEVICE_ID, InterruptController, &GicConfig);
...
int ScuGicExample(u16 DeviceId, XScuGic InterruptController, XScuGic_Config *GicConfig)
{
    ...
    Status = XScuGic_Connect(&InterruptController, INTC_DEVICE_INT_ID_BIS,
			   (Xil_ExceptionHandler)DeviceDriverHandlerBis,
			   (void *)&InterruptController);
	 ...
	 * interrupt so the handlers will be called
	 */
	XScuGic_Enable(&InterruptController, INTC_DEVICE_INT_ID_BIS);
...
void DeviceDriverHandlerBis(void *CallbackRef)
{
	 ...
	 * Indicate the interrupt has been processed using a shared variable
	 */
    xil_printf("****** Handler bis ****** \n");
    //*CallBackRef=displayIntcPriority;
	InterruptProcessed = TRUE;
}

void displayIntcPriority(u8 priority)
{
    xil_printf("****** Priority = %u ****** \n", priority);
}